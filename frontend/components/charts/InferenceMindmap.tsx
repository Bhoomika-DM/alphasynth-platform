...
const CustomNode = memo(({ data }: any) => {
  ...
  const getNodeStyle = () => {
    switch (type) {
      ...
      default:
        return 'bg-[#0D7C8C]/10 border-2 border-[#0D7C8C]/40 text-[#0D7C8C]'
    }
  }
  ...
  return (
    <div className={`relative px-5 py-3 rounded-xl ${getNodeStyle()} shadow-sm transition-all hover:shadow-md`}>
      <Handle 
        ...
        className="!w-2 !h-2 !bg-[#0D7C8C] !border-2 !border-white"
      />
      <Handle 
        type="source" 
...
function InferenceMindmapFlow({ data }: InferenceMindmapProps) {
  ...
  const initialEdges: Edge[] = useMemo(() => [
    ...
    { id: 'e1', source: 'macro1', target: 'business1', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e2', source: 'macro1', target: 'business2', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e3', source: 'macro2', target: 'business2', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e4', source: 'macro2', target: 'business3', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e5', source: 'macro3', target: 'business3', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e6', source: 'macro3', target: 'business4', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },

    // Business → PVI (all 4 business to PVI)
    { id: 'e7', source: 'business1', target: 'pvi', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e8', source: 'business2', target: 'pvi', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e9', source: 'business3', target: 'pvi', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
    { id: 'e10', source: 'business4', target: 'pvi', type: 'smoothstep', style: { stroke: '#0D7C8C', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },

    // PVI → Valuation
    { id: 'e11', source: 'pvi', target: 'valuation', type: 'smoothstep', animated: true, style: { stroke: '#0D7C8C', strokeWidth: 3 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#0D7C8C' } },
  ...
  return (
    <div className="relative w-full h-[600px] bg-white rounded-xl border-2 border-[#2E4D8E]/10">
      <ReactFlow
        nodes={nodes}
        edges={edges}
      ...
      >
        <Background color="#0D7C8C" gap={20} size={1} className="opacity-10" />
        <Controls className="bg-white border border-[#2E4D8E]/20 rounded-lg" />
      </ReactFlow>

      {/* Legend */}
      <div className="absolute top-4 right-4 bg-white border border-[#2E4D8E]/20 rounded-lg p-4 shadow-sm">
        <div className="text-xs font-bold text-[#2D3748] mb-3 uppercase tracking-wider">Legend</div>
        <div className="space-y-2 text-xs">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-[#3B82F6]/20 border border-[#3B82F6]/40"></div>
            <span className="text-[#718096]">Macro Drivers</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-[#10B981]/20 border border-[#10B981]/40"></div>
            <span className="text-[#718096]">Business Drivers</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-[#F59E0B]/20 border border-[#F59E0B]/40"></div>
            <span className="text-[#718096]">PVI Impact</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-[#8B5CF6]/20 border border-[#8B5CF6]/40"></div>
            <span className="text-[#718096]">Valuation</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-[#EF4444]/20 border border-[#EF4444]/40"></div>
            <span className="text-[#718096]">Final Verdict</span>
          </div>
        </div>
      </div>
    </div>
