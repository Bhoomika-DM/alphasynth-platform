...
export default function LightweightAreaChart({ data, height = 320, isPositive = true }: LightweightAreaChartProps) {
  ...
  useEffect(() => {
    ...
    if (chartRef.current) {
      ...
      if (seriesRef.current) {
        const lineColor = isPositive ? '#0D7C8C' : '#C85A54' // Sage green for profit, Terracotta for loss
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      crosshair: {
        vertLine: {
          color: isPositive ? '#0D7C8C' : '#C85A54',
          width: 1,
          style: 3,
          labelBackgroundColor: isPositive ? '#0D7C8C' : '#C85A54',
        },
        horzLine: {
          color: isPositive ? '#0D7C8C' : '#C85A54',
          width: 1,
  ...
  useEffect(() => {
    if (seriesRef.current && chartRef.current) {
      const lineColor = isPositive ? '#0D7C8C' : '#C85A54'
      const topColor = isPositive ? 'rgba(107, 158, 93, 0.4)' : 'rgba(200, 90, 84, 0.4)'
      const bottomColor = isPositive ? 'rgba(107, 158, 93, 0.05)' : 'rgba(200, 90, 84, 0.05)'
