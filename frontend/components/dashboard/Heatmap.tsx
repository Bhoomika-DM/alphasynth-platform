  ...
  return (
    <div ref={cardRef} className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-2xl font-bold text-white mb-1">Sector Performance</h4>
          <p className="text-base text-white/80">Live market overview</p>
        </div>
        <div className="flex items-center gap-3">
          {/* Time Period Filters */}
          <div className="flex items-center gap-1 p-1 bg-white border border-[#2E4D8E]/10 rounded-md">
            {periods.map((period) => (
              <button
                key={period}
                ...
                className={`px-4 py-2 rounded-md text-sm font-semibold transition-all duration-200 ${
                  selectedPeriod === period
                    ? 'bg-[#0D7C8C] text-[#2D3748] border border-[#2E4D8E]/20'
      ...
      <div className="bg-gradient-to-br from-white/[0.07] to-white/[0.02] backdrop-blur-2xl border border-white/[0.08] rounded-2xl overflow-hidden">
        ...
        <div className="grid grid-cols-5 gap-4 px-6 py-4 bg-white/[0.03] border-b border-white/[0.08]">
          <div className="text-sm font-bold text-white uppercase tracking-wider">Sector</div>
          <div className="text-sm font-bold text-white uppercase tracking-wider">Change</div>
          <div className="text-sm font-bold text-white uppercase tracking-wider">Top Stocks</div>
          <div className="text-sm font-bold text-white uppercase tracking-wider">Volume</div>
          <div className="text-sm font-bold text-white uppercase tracking-wider">Trend</div>
        </div>
        ...
        <div className="divide-y divide-white/[0.05]">
          {sectors.map((sector, idx) => (
            ...
            >
              ...
              {/* Top Stocks */}
              <div className="flex items-center">
                <span className="text-sm text-white/80">{sector.stocks}</span>
              </div>

              {/* Volume */}
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${getVolumeColor(sector.volume)}`} />
                <span className="text-sm text-white">{sector.volume}</span>
              </div>
      ...
      <div className="grid grid-cols-5 gap-4">
        <div className="p-4 bg-white border border-[#2E4D8E]/20 rounded-lg">
          <div className="text-sm text-[#718096] mb-1 font-medium">Top Gainer</div>
          <div className="text-xl font-black text-[#2E4D8E]">
        ...
        <div className="p-4 bg-white border border-[#8C1A1A]/20 rounded-lg">
          <div className="text-sm text-[#718096] mb-1 font-medium">Top Loser</div>
          <div className="text-xl font-black text-[#8C1A1A]">
            {sectors.reduce((min, s) => s.change < min.change ? s : min).name.split(' ')[0]} {sectors.reduce((min, s) => s.change < min.change ? s : min).change}%
          </div>
        </div>
        <div className="p-4 bg-white border border-[#2E4D8E]/20 rounded-lg">
          <div className="text-sm text-[#718096] mb-1 font-medium">Advancing</div>
          <div className="text-xl font-black text-[#2D3748]">{sectors.filter(s => s.change > 0).length} sectors</div>
        </div>
        <div className="p-4 bg-white border border-[#B8860B]/20 rounded-lg">
          <div className="text-sm text-[#718096] mb-1 font-medium">Neutral</div>
          <div className="text-xl font-black text-[#B8860B]">{sectors.filter(s => s.change === 0).length} sectors</div>
        </div>
        <div className="p-4 bg-white border border-[#8C1A1A]/20 rounded-lg">
          <div className="text-sm text-[#718096] mb-1 font-medium">Declining</div>
          <div className="text-xl font-black text-[#2D3748]">{sectors.filter(s => s.change < 0).length} sector{sectors.filter(s => s.change < 0).length !== 1 ? 's' : ''}</div>
        </div>
      </div>
    </div>
  )
