  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-2xl font-bold text-[#2D3748] mb-1">Analyze Basket</h4>
          <p className="text-base text-[#718096]">Select index baskets to analyze performance and statistics</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h5 className="text-lg font-bold text-[#2D3748] flex items-center gap-2">
            <IconChartBar className="w-5 h-5 text-[#2E4D8E]" stroke={1.5} />
            India Index Baskets
          </h5>
            ...
            return (
              <button
                ...
                className={`group relative p-4 rounded-xl border-2 transition-all duration-200 ${
                  isSelected
                    ? 'bg-[#2E4D8E]/10 border-[#2E4D8E] shadow-md'
                    : 'bg-white border-[#2E4D8E]/20 hover:border-[#2E4D8E]/40 hover:bg-[#F8F9FB]'
                }`}
              >
                {/* Selection indicator */}
                <div className={`absolute top-2 right-2 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all duration-200 ${
                  isSelected
                    ? 'border-[#2E4D8E] bg-[#2E4D8E]'
                    : 'border-[#2E4D8E]/30 bg-transparent'
                }`}>
                  {isSelected && (
                ...
                <div className="text-left">
                  <div className={`text-sm font-bold mb-1 transition-colors ${
                    isSelected ? 'text-[#2E4D8E]' : 'text-[#2D3748]'
                  }`}>
                    {basket.name}
                  </div>
      ...
      <div className="relative bg-white border-2 border-[#2E4D8E]/20 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h5 className="text-lg font-bold text-[#2D3748] mb-1">Analyze a custom portfolio</h5>
            <p className="text-sm text-[#718096]">Enter any combination of symbols, names or groups</p>
          </div>
          <button
            onClick={handleCustomAnalysis}
            disabled={selectedBaskets.length === 0}
            className={`px-6 py-3 rounded-md font-semibold text-sm transition-all duration-200 border-2 ${
              selectedBaskets.length > 0
                ? 'bg-[#0D7C8C] border-[#2E4D8E]/30 text-[#2D3748] hover:bg-[#2E4D8E] hover:text-white'
                : 'bg-[#F8F9FB] border-[#2E4D8E]/10 text-[#718096] cursor-not-allowed'
            }`}
          >
            Custom Analysis
          </button>
      ...
      {selectedBaskets.length > 0 && (
        <div className="relative bg-[#2E4D8E]/5 border-2 border-[#2E4D8E]/30 rounded-2xl p-5 shadow-sm">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h5 className="text-sm font-bold text-[#2D3748] mb-1">Selected Baskets</h5>
            ...
            <button
              ...
              className="text-xs text-[#718096] hover:text-[#2D3748] transition-colors"
            >
              Clear all
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {selectedBaskets.map((basketId) => {
              ...
              return (
                <div
                  key={basketId}
                  className="flex items-center gap-2 px-3 py-2 bg-white border-2 border-[#2E4D8E]/30 rounded-md"
                >
                  <span className="text-xs font-semibold text-[#2E4D8E]">
                    {basket.name}
                  </span>
                  <button
                    onClick={() => toggleBasket(basketId)}
                    className="p-0.5 hover:bg-[#F8F9FB] rounded transition-colors"
                  >
                    <IconX className="w-3 h-3 text-[#718096] hover:text-[#2D3748]" stroke={1.5} />
                  </button>
                </div>
              )
            })}
