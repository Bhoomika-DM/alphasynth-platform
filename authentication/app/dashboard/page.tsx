...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (shouldShowOnboardingFromStorage === 'true' || shouldShowOnboardingFromUrl === 'true') {
  ...
  if (loading || indicesLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F4F7F2]">
        <div className="relative z-10 text-[#1F2933] font-jakarta">Loading real market data...</div>
      </div>
    )
  }

  // Sage & Gold Color Palette
  const colors = {
    sageGreen: '#6B9E5D',      // Light sage for positive values
    sageDark: '#5A8A4E',       // Dark sage for headers
    gold: '#E5C76A',           // Gold for accents
    terracotta: '#C85A54',     // Red for losses
    background: '#F4F7F2',     // Light cream background
    text: '#1F2933',           // Black text
  ...
  if (niftyIndices.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F4F7F2]">
        <div className="relative z-10 text-[#1F2933] font-jakarta">Unable to load market data. Please refresh.</div>
      </div>
    )
  }
    ...
    <ErrorBoundary>
      ...
      {showMessage && message && (
        <div className="fixed top-6 right-6 z-50 animate-slide-in">
          <div className="bg-[#6B9E5D] rounded-lg shadow-lg p-4 flex items-center gap-3 border border-[#5A8A4E]/30">
            <IconCircleCheck className="w-5 h-5 text-[#5A8A4E]" stroke={1.5} />
            <p className="text-[#1F2933] font-jakarta font-semibold">{message}</p>
            <button onClick={() => setShowMessage(false)} className="text-[#6B7280] hover:text-[#1F2933]">
      ...
      {/* Top Ticker - Stocks */}
      <div className="bg-white border-b border-[#5A8A4E]/20 py-2 overflow-hidden">
        <div className="flex items-center gap-4 animate-scroll">
          {[...stockTickers, ...stockTickers].map((stock, idx) => (
            <a
              ...
              className="flex items-center gap-3 whitespace-nowrap hover:bg-[#F4F7F2] px-3 py-1 rounded transition-colors cursor-pointer"
            >
              <div className="w-0.5 h-6 bg-[#5A8A4E]"></div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-jakarta font-bold text-[#1F2933]">{stock.symbol}</span>
                <span className="text-sm font-jakarta font-semibold text-[#1F2933]">{stock.price.toFixed(2)}</span>
                <span className="text-sm font-jakarta font-semibold text-[#6B9E5D]">
      ...
      <div className="bg-[#F4F7F2] border-b border-[#5A8A4E]/20 py-3 relative z-10">
          ...
          <button
            ...
            className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all z-20 ${
              niftyStartIndex === 0
                ...
                : 'bg-[#A7C4A0] text-[#1F2933] hover:bg-[#5A7A61] cursor-pointer active:scale-95'
            }`}
            type="button"
              ...
              return (
                <button
                  ...
                  className={`text-center p-2 rounded transition-all cursor-pointer ${
                    isSelected 
                      ? 'bg-[#6A994E]/10 border-2 border-[#6A994E] shadow-md' 
                      : 'bg-transparent hover:bg-[#F4F7F2] border-2 border-transparent'
                  }`}
                  type="button"
                  aria-label={`Select ${index.name}`}
                >
                  <div className={`text-xs font-jakarta font-bold uppercase mb-1 ${
                    isSelected ? 'text-[#6A994E]' : 'text-[#6B7280]'
                  }`}>
                    {index.name}
                  </div>
                  <div className="text-lg font-jakarta font-black text-[#1F2933] mb-0.5">
                    {index.value.toLocaleString()}
                  </div>
                  <div className="text-sm font-jakarta font-semibold text-[#6B9E5D]">
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 p-4">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2">
                <h3 className="text-base font-jakarta font-bold text-[#1F2933] uppercase">
                  {selectedIndex.name}
                </h3>
                <IconTrendingUp className="w-4 h-4 text-[#6B9E5D]" stroke={1.5} />
              </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div>
                <div className="text-3xl font-jakarta font-black text-[#1F2933] leading-none mb-1">
              ...
              <div className="flex items-center gap-8">
                <div>
                  <div className="flex items-center gap-1.5 mb-0.5">
                    ...
                    <div className="text-xs text-[#6B7280] font-jakarta">Open</div>
                  </div>
                  <div className="text-sm text-[#1F2933] font-jakarta font-semibold">
                    {selectedIndex.open ? selectedIndex.open.toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '-'}
                  </div>
                </div>
                <div>
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <div className="w-2 h-2 rounded-full bg-[#6B9E5D]"></div>
                    <div className="text-xs text-[#6B7280] font-jakarta">High</div>
                  </div>
                  <div className="text-sm text-[#6B9E5D] font-jakarta font-semibold">
                    {selectedIndex.high ? selectedIndex.high.toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '-'}
                  </div>
                </div>
                <div>
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <div className="w-2 h-2 rounded-full bg-[#C85A54]"></div>
                    <div className="text-xs text-[#6B7280] font-jakarta">Low</div>
                  </div>
                  <div className="text-sm text-[#C85A54] font-jakarta font-semibold">
              ...
              <div className="flex gap-2">
                {periods.map((period) => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-md text-xs font-jakarta font-semibold transition-all ${
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-jakarta font-black text-[#1F2933]">Market Statistics</h3>
              <div className="text-sm text-[#6B7280] font-jakarta">As on {marketStats.date} 16:00 IST</div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-[#F4F7F2] rounded-lg p-3 border-l-4 border-[#4A5568] hover:shadow-lg transition-all cursor-pointer"
              >
                <div className="text-xs text-[#6B7280] font-jakarta mb-1">Stock Traded</div>
                <div className="text-2xl font-jakarta font-black text-[#4A5568]">{marketStats.stockTraded}</div>
              </a>
              <a 
                href="https://www.nseindia.com/market-data/advance-decline-market-breadth" 
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-[#F4F7F2] rounded-lg p-3 border-l-4 border-[#6B9E5D] hover:shadow-lg transition-all cursor-pointer"
              >
                <div className="text-xs text-[#6B7280] font-jakarta mb-1">Advances</div>
                <div className="text-2xl font-jakarta font-black text-[#6B9E5D]">{marketStats.advances}</div>
              </a>
              <a 
                href="https://www.nseindia.com/market-data/advance-decline-market-breadth" 
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-[#F4F7F2] rounded-lg p-3 border-l-4 border-[#C85A54] hover:shadow-lg transition-all cursor-pointer"
              >
                <div className="text-xs text-[#6B7280] font-jakarta mb-1">Declines</div>
                <div className="text-2xl font-jakarta font-black text-[#C85A54]">{marketStats.declines}</div>
              </a>
              <a 
                href="https://www.nseindia.com/market-data/advance-decline-market-breadth" 
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-[#F4F7F2] rounded-lg p-3 border-l-4 border-[#E5C76A] hover:shadow-lg transition-all cursor-pointer"
              >
                <div className="text-xs text-[#6B7280] font-jakarta mb-1">Unchanged</div>
                <div className="text-2xl font-jakarta font-black text-[#E5C76A]">{marketStats.unchanged}</div>
              </a>
            </div>

            {/* Middle Row: 52 Week Stats + Logo + Circuit Stats - Now Clickable */}
            <div className="bg-[#F4F7F2] rounded-lg p-4 mb-4">
              <div className="grid grid-cols-5 gap-4 items-center">
                ...
                >
                  <div className="text-xs text-[#6B7280] font-jakarta mb-1">No. of Stocks at</div>
                  <div className="text-sm font-jakarta font-bold text-[#1F2933] mb-1">52 Week High</div>
                  <div className="flex items-center gap-1">
                    <IconArrowUp className="w-4 h-4 text-[#6B9E5D]" stroke={2} />
                    <span className="text-2xl font-jakarta font-black text-[#4A5568]">{marketStats.week52High}</span>
                  </div>
                </a>
                ...
                >
                  <div className="text-xs text-[#6B7280] font-jakarta mb-1">No. of Stocks at</div>
                  <div className="text-sm font-jakarta font-bold text-[#1F2933] mb-1">52 Week Low</div>
                  <div className="flex items-center gap-1">
                    <IconArrowDown className="w-4 h-4 text-[#C85A54]" stroke={2} />
                    <span className="text-2xl font-jakarta font-black text-[#4A5568]">{marketStats.week52Low}</span>
                  </div>
                </a>
                ...
                <a 
                  ...
                  className="hover:bg-white rounded-lg p-2 transition-all cursor-pointer"
                >
                  <div className="text-xs text-[#6B7280] font-jakarta mb-1">No. of Stocks in</div>
                  <div className="text-sm font-jakarta font-bold text-[#1F2933] mb-1">Upper Circuit</div>
                  <div className="text-2xl font-jakarta font-black text-[#6B9E5D]">{marketStats.upperCircuit}</div>
                </a>
                ...
                >
                  <div className="text-xs text-[#6B7280] font-jakarta mb-1">No. of Stocks in</div>
                  <div className="text-sm font-jakarta font-bold text-[#1F2933] mb-1">Lower Circuit</div>
                  <div className="text-2xl font-jakarta font-black text-[#C85A54]">{marketStats.lowerCircuit}</div>
                </a>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-[#F4F7F2] rounded-lg p-4 border-l-4 border-[#E5C76A] hover:shadow-lg transition-all cursor-pointer"
              >
                <div className="text-sm text-[#4A5568] font-jakarta font-bold mb-2">Registered Investors</div>
                <div className="text-2xl font-jakarta font-black text-[#4A5568]">{marketStats.registeredInvestors}</div>
              </a>
              <div className="bg-[#F4F7F2] rounded-lg p-4 border-l-4 border-[#E5C76A]">
                <div className="text-sm text-[#4A5568] font-jakarta font-bold mb-2">Market Capitalization</div>
                <div className="text-lg font-jakarta font-black text-[#4A5568]">{marketStats.marketCap}</div>
                <div className="text-xs text-[#6B7280] font-jakarta mt-1">{marketStats.date}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-[#5A8A4E]/20">
              <div className="flex items-center gap-4">
                <h3 className="text-xl font-jakarta font-black text-[#1F2933]">Market Turnover</h3>
                <span className="text-sm text-[#6B7280] font-jakarta">As on {marketStats.date}</span>
              </div>
              <a 
                href="https://www.nseindia.com/market-data/live-market-indices" 
                target="_blank" 
                rel="noopener noreferrer"
                className="px-4 py-2 bg-white border border-[#E5E7EB] rounded-md text-sm font-jakarta font-semibold text-[#1F2933] hover:bg-[#F4F7F2] transition-colors flex items-center gap-2 cursor-pointer relative z-10"
              >
                View More
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-[#A7C4A0] text-[#1F2933]">
                    <th className="text-left py-3 px-4 text-sm font-jakarta font-bold uppercase">PRODUCTS</th>
                    <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">
                      <div>VOLUME</div>
                    ...
                    <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">UPDATED AT</th>
                  </tr>
                </thead>
                <tbody>
                  {marketTurnover.map((item, index) => (
                    <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'} hover:bg-[#F4F7F2] transition-colors`}>
                      <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933]">{item.product}</td>
                      <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{item.volume}</td>
                      <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{item.value}</td>
                      <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{item.openInterest}</td>
                      <td className="py-3 px-4 text-sm font-jakarta text-[#6B7280] text-right">{item.updatedAt}</td>
                    </tr>
                  ))}
                  {/* Total Row */}
                  <tr className="bg-[#F4F7F2] border-t-2 border-[#5A8A4E]/30">
                    <td className="py-3 px-4 text-sm font-jakarta font-black text-[#1F2933]">Total</td>
                    <td className="py-3 px-4 text-sm font-jakarta font-black text-[#1F2933] text-right">145.34 Cr</td>
                    <td className="py-3 px-4 text-sm font-jakarta font-black text-[#1F2933] text-right">92,659.03</td>
                    <td className="py-3 px-4 text-sm font-jakarta font-black text-[#1F2933] text-right">2.63 Cr</td>
                    <td className="py-3 px-4 text-sm font-jakarta text-right"></td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-[#5A8A4E]/20">
              <div className="flex items-center gap-4">
                <h3 className="text-xl font-jakarta font-black text-[#1F2933]">Market Snapshot</h3>
                <span className="text-sm text-[#6B7280] font-jakarta">As on {marketStats.date} 16:00 IST</span>
              </div>
              <a 
            ...
            <div className="flex gap-0 border-b-2 border-[#E5E7EB] px-4">
              {['Gainers', 'Losers', 'Most Active(Value)', 'Most Active(Volume)', 'ETFs(Volume)'].map((tab, index) => (
                <button
                  ...
                  className={`px-4 py-3 text-sm font-jakarta font-bold transition-all relative ${
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="bg-[#A7C4A0] text-[#1F2933]">
                    <th className="text-left py-3 px-4 text-sm font-jakarta font-bold uppercase">SYMBOL</th>
                    <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">LTP</th>
                    <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">CHNG</th>
                    <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">%CHNG</th>
                    <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">
                      <div>VOLUME</div>
                ...
                <tbody>
                  {currentMarketSnapshot.map((stock, index) => {
                    ...
                    return (
                      <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'} hover:bg-[#F4F7F2] transition-colors`}>
                        <td className="py-3 px-4 text-sm font-jakarta font-semibold text-[#5A8A4E]">{stock.symbol}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right font-semibold">{stock.ltp.toFixed(2)}</td>
                        <td className={`py-3 px-4 text-sm font-jakarta text-right font-semibold ${isPositive ? 'text-[#6B9E5D]' : 'text-[#C85A54]'}`}>
                          {isPositive ? '+' : ''}{stock.change.toFixed(2)}
                        </td>
                        <td className={`py-3 px-4 text-sm font-jakarta text-right font-semibold ${isPositive ? 'text-[#6B9E5D]' : 'text-[#C85A54]'}`}>
                          {isPositive ? '+' : ''}{stock.changePercent.toFixed(2)}
                        </td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{stock.volume.toFixed(2)}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{stock.value.toFixed(2)}</td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            <div className="px-4 py-3 bg-[#F9FAFB] border-t border-[#E5E7EB]">
              <p className="text-xs text-[#6B7280] font-jakarta italic">
        ...
        <div className="bg-white rounded-lg border border-[#5A8A4E]/20 overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 border-b border-[#5A8A4E]/20">
            <div className="flex items-center gap-4">
              <h3 className="text-xl font-jakarta font-black text-[#1F2933]">Most Active Contracts</h3>
              <span className="text-sm text-[#6B7280] font-jakarta">As on 07-Apr-2026 11:41 IST</span>
            </div>
            <a 
          ...
          <div className="grid grid-cols-3 gap-0">
            ...
            <div className="border-r border-[#5A8A4E]/20">
              <div className="bg-[#F4F7F2] px-4 py-2 border-b border-[#5A8A4E]/20">
                <h4 className="text-sm font-jakarta font-bold text-[#1F2933]">Most Active Calls</h4>
              </div>
              <div className="max-h-[400px] overflow-y-auto table-scroll">
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-[#A7C4A0] text-[#1F2933]">
                      <th className="text-left py-2 px-3 text-xs font-jakarta font-bold uppercase">CONTRACT</th>
                      <th className="text-right py-2 px-3 text-xs font-jakarta font-bold uppercase">LTP</th>
                      <th className="text-right py-2 px-3 text-xs font-jakarta font-bold uppercase">CHNG</th>
                      <th className="text-right py-2 px-3 text-xs font-jakarta font-bold uppercase">%CHNG</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'}`}>
                        <td className="py-2 px-3 text-xs font-jakarta text-[#4A5568]">{item.contract}</td>
                        <td className="py-2 px-3 text-xs font-jakarta text-[#1F2933] text-right">{item.ltp.toFixed(2)}</td>
                        <td className="py-2 px-3 text-xs font-jakarta text-[#C85A54] text-right">{item.chng.toFixed(2)}</td>
                        <td className="py-2 px-3 text-xs font-jakarta text-[#C85A54] text-right">{item.chngPercent.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="border-r border-[#5A8A4E]/20">
              <div className="bg-[#F4F7F2] px-4 py-2 border-b border-[#5A8A4E]/20">
                <h4 className="text-sm font-jakarta font-bold text-[#1F2933]">Most Active Puts</h4>
              </div>
              <div className="max-h-[400px] overflow-y-auto table-scroll">
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
            ...
            <div>
              <div className="bg-[#F4F7F2] px-4 py-2 border-b border-[#5A8A4E]/20">
                <h4 className="text-sm font-jakarta font-bold text-[#1F2933]">Most Active Contracts by OI</h4>
              </div>
              <div className="max-h-[400px] overflow-y-auto table-scroll">
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-[#5A8A4E]/20">
              <h3 className="text-xl font-jakarta font-black text-[#1F2933]">Corporate Info</h3>
              <a 
                  ...
                  {[
                    ...
                    <div key={index} className="flex items-start justify-between gap-4 pb-4 border-b border-[#E5E7EB] last:border-0">
                      <div className="flex-1">
                        <div className="text-sm font-jakarta font-bold text-[#1F2933] mb-1">{item.company}</div>
                ...
                <>
                  {[
                    ...
                    <div key={index} className="pb-4 border-b border-[#E5E7EB] last:border-0 border-l-4 border-[#E5C76A] pl-3">
                      <div className="text-sm font-jakarta font-bold text-[#1F2933] mb-2">{item.company}</div>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div><span className="font-semibold text-[#6B7280]">Purpose:</span> <span className="text-[#1F2933]">{item.purpose}</span></div>
                        <div><span className="font-semibold text-[#6B7280]">Series:</span> <span className="text-[#1F2933]">{item.series}</span></div>
                        <div><span className="font-semibold text-[#6B7280]">Ex-Date:</span> <span className="text-[#1F2933]">{item.exDate}</span></div>
                        <div><span className="font-semibold text-[#6B7280]">Record Date:</span> <span className="text-[#1F2933]">{item.recordDate}</span></div>
                      </div>
                    </div>
                  ))}
                </>
              ...
              {corporateTab === 2 && (
                <>
                  {[
                    ...
                    <div key={index} className="flex items-start justify-between gap-4 pb-4 border-b border-[#E5E7EB] last:border-0 border-l-4 border-[#6B9E5D] pl-3">
                      <div className="flex-1">
                        <div className="text-sm font-jakarta font-bold text-[#1F2933] mb-2">{item.company}</div>
                        <div className="grid grid-cols-2 gap-2 text-xs mb-2">
                          <div><span className="font-semibold text-[#6B7280]">Audited/Unaudited:</span> <span className="text-[#1F2933]">{item.audited}</span></div>
                          <div><span className="font-semibold text-[#6B7280]">Cumulative/Non-Cumulative:</span> <span className="text-[#1F2933]">{item.cumulative}</span></div>
                        </div>
                        <div className="text-xs"><span className="font-semibold text-[#6B7280]">Broadcast Date:</span> <span className="text-[#1F2933]">{item.broadcastDate}</span></div>
                      </div>
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-[#5A8A4E]/20">
              <h3 className="text-xl font-jakarta font-black text-[#1F2933]">IPO Tracker</h3>
              <a 
            ...
            <div className="p-4 grid grid-cols-2 gap-4">
              ...
              >
                <div>
                  <div className="text-3xl font-jakarta font-black text-[#4A5568] mb-1">219</div>
                  <div className="text-xs text-[#6B7280] font-jakarta">IPOs in the Year</div>
                </div>
              ...
              >
                <div>
                  <div className="text-3xl font-jakarta font-black text-[#4A5568] mb-1">109</div>
                  <div className="text-xs text-[#6B7280] font-jakarta">IPOs on NSE SME Segment</div>
                </div>
                <IconChartBar className="w-12 h-12 text-[#5B4B8A] opacity-60" stroke={1.5} />
              </a>
              ...
              >
                <div>
                  <div className="text-3xl font-jakarta font-black text-[#6B9E5D] mb-1">84</div>
                  <div className="text-xs text-[#6B7280] font-jakarta">IPOs in Gain w.r.t. Issue Price</div>
                </div>
                <IconChartLine className="w-12 h-12 text-[#6B9E5D] opacity-60" stroke={1.5} />
              </a>
              ...
              >
                <div>
                  <div className="text-3xl font-jakarta font-black text-[#C85A54] mb-1">135</div>
                  <div className="text-xs text-[#6B7280] font-jakarta">IPOs in Loss w.r.t. Issue Price</div>
                </div>
              ...
              >
                <div>
                  <div className="text-3xl font-jakarta font-black text-[#6B9E5D] mb-1">137</div>
                  <div className="text-xs text-[#6B7280] font-jakarta">IPOs in Gain on Listing Date</div>
                </div>
                <IconChartLine className="w-12 h-12 text-[#6B9E5D] opacity-60" stroke={1.5} />
              </a>
              ...
              >
                <div>
                  <div className="text-3xl font-jakarta font-black text-[#C85A54] mb-1">82</div>
                  <div className="text-xs text-[#6B7280] font-jakarta">IPOs in Loss on Listing Date</div>
                </div>
                <IconChartLine className="w-12 h-12 text-[#C85A54] opacity-60" stroke={1.5} />
              </a>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-[#5A8A4E]/20">
              <div className="flex items-center gap-4">
                <h3 className="text-xl font-jakarta font-black text-[#1F2933]">Reference Rates</h3>
                <span className="text-sm text-[#6B7280] font-jakarta">As on 06-Apr-2026 13:00:00 IST</span>
              </div>
              <a 
            ...
            <div className="max-h-[400px] overflow-y-auto table-scroll">
              ...
              {referenceRatesTab === 0 && (
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-[#A7C4A0] text-[#1F2933]">
                      <th className="text-left py-3 px-4 text-sm font-jakarta font-bold uppercase">CURRENCY</th>
                      <th className="text-center py-3 px-4 text-sm font-jakarta font-bold uppercase">UNIT</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">TODAY'S VALUE</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">PREVIOUS DAY VALUE</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'} hover:bg-[#F4F7F2] transition-colors`}>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] flex items-center gap-2">
                          <span className="text-lg">{item.flag}</span>
                          <span className="font-semibold">{item.currency}</span>
                        </td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-center">{item.unit}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right font-semibold">{item.today}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#6B7280] text-right">{item.previous}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {referenceRatesTab === 1 && (
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-[#A7C4A0] text-[#1F2933]">
                      <th className="text-left py-3 px-4 text-sm font-jakarta font-bold uppercase">UNDERLYING</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">PRICE</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">YIELD</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'} hover:bg-[#F4F7F2] transition-colors`}>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#4A5568] font-semibold">{item.underlying}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right font-semibold">{item.price}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right font-semibold">{item.yield}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-[#A7C4A0] text-[#1F2933]">
                      <th className="text-left py-3 px-4 text-sm font-jakarta font-bold uppercase">SYMBOL</th>
                      <th className="text-center py-3 px-4 text-sm font-jakarta font-bold uppercase">UNIT</th>
                      <th className="text-center py-3 px-4 text-sm font-jakarta font-bold uppercase">UPDATED DATE</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">SPOT PRICE (IN ₹)<br/>POLLED AT 12:30 HRS</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">SPOT PRICE (IN ₹)<br/>POLLED AT 16:30 HRS</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'} hover:bg-[#F4F7F2] transition-colors`}>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#4A5568] font-semibold">{item.symbol}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-center">{item.unit}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-center">{item.date}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right font-semibold">{item.price1230}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right font-semibold">{item.price1630}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="bg-white rounded-lg border border-[#5A8A4E]/20 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-[#5A8A4E]/20">
              <div className="flex items-center gap-4">
                <h3 className="text-xl font-jakarta font-black text-[#1F2933]">Currency Snapshot</h3>
                <span className="text-sm text-[#6B7280] font-jakarta">As on 07-Apr-2026 12:13 IST</span>
              </div>
              <a 
            ...
            <div className="max-h-[400px] overflow-y-auto table-scroll">
              ...
              {currencySnapshotTab === 0 && (
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-[#A7C4A0] text-[#1F2933]">
                      <th className="text-left py-3 px-4 text-sm font-jakarta font-bold uppercase">CONTRACTS</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">LTP</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">%CHNG</th>
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">
                        <div>VOLUME</div>
                      ...
                      <th className="text-right py-3 px-4 text-sm font-jakarta font-bold uppercase">OPEN INTEREST</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'} hover:bg-[#F4F7F2] transition-colors`}>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#4A5568] font-semibold">{item.contract}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right font-semibold">{item.ltp}</td>
                        <td className={`py-3 px-4 text-sm font-jakarta text-right font-semibold ${item.change >= 0 ? 'text-[#6B9E5D]' : 'text-[#C85A54]'}`}>
                          {item.change >= 0 ? '+' : ''}{item.change.toFixed(4)}
                        </td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{item.volume}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{item.turnover}</td>
                        <td className="py-3 px-4 text-sm font-jakarta text-[#1F2933] text-right">{item.oi}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
