...
const CSS_RANGES: CSSRange[] = [
  ...
  },
  {
    ...
    color: 'text-[#A7C4A0]',
    bgColor: 'from-[#A7C4A0]/20 to-[#A7C4A0]/5',
    borderColor: 'border-[#A7C4A0]/30'
  }
]
...
export default function CSSAdjustmentGuide() {
  return (
    <div className="space-y-4">
      ...
      >
        <p className="text-sm font-jakarta text-white/80 leading-relaxed mb-3">
      ...
      <div className="space-y-3">
        {CSS_RANGES.map((range, index) => (
          ...
          >
            {range.isHighlight && (
              <div className="absolute -top-2 -right-2 px-3 py-1 bg-yellow-500 text-black text-xs font-jakarta font-bold rounded-full animate-pulse">
                CURRENT
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <span className={`text-lg font-jakarta font-black ${range.color}`}>
                      CSS {range.range}
                    </span>
                    <span className={`text-base font-jakarta font-bold ${range.color}`}>
                      {range.label}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-black/20 rounded-lg">
                  <div className="text-xs font-jakarta text-green-400 font-bold mb-1">
                    {range.buySignal}
                  </div>
                </div>
                <div className="p-3 bg-black/20 rounded-lg">
                  <div className="text-xs font-jakarta text-red-400 font-bold mb-1">
                    {range.sellSignal}
                  </div>
                </div>
              </div>
              ...
              {/* Description */}
              <div className="text-sm font-jakarta text-white/70 italic">
                {range.description}
              </div>
            </div>
          </motion.div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="text-xs font-jakarta text-blue-400 font-bold">How to Use This Guide:</div>
        </div>
        <ul className="space-y-1 text-xs font-jakarta text-white/70">
