...
const CustomNode = memo(({ data }: any) => {
  ...
  return (
    <div className={`relative px-6 py-4 rounded-2xl bg-gradient-to-br ${getColor()} shadow-2xl border-2 border-white/20`}>
      ...
      <div className={`font-sans font-bold text-white ${getTextSize()} text-center whitespace-nowrap`}>
        {label}
      </div>
      {score !== undefined && (
        <div className="text-xs font-sans text-white/90 text-center mt-1">
          {score}
        </div>
      )}
    </div>
  ...
  const ExplanationPanel = () => (
    <div className="absolute top-4 left-4 z-10 bg-black/90 backdrop-blur-xl border border-green-500/30 rounded-lg p-4 max-w-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="text-sm font-sans font-bold text-white">INSIGHT</div>
      </div>
      ...
      <div className="fixed inset-0 z-[100] bg-black">
        <div className="relative w-full h-full">
          ...
          <div className="absolute bottom-4 left-4 z-10 bg-black/80 backdrop-blur-xl border border-white/20 rounded-lg p-4">
            <div className="text-xs font-sans font-bold text-white mb-2">SCORE LEGEND</div>
            <div className="space-y-1 text-xs font-sans text-white/80">
