    ...
    <div ref={containerRef} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
      {marketData.map((item, index) => (
        <a
          ...
          className="market-card group relative bg-white border border-[#6A994E]/10 rounded-lg p-4 hover:border-[#6A994E]/30 hover:shadow-md transition-all duration-300 cursor-pointer hover:-translate-y-0.5 overflow-hidden block"
        >
          <div className="relative flex items-center gap-4">
            {/* Left side - Data */}
            <div className="flex-1 space-y-2">
              <div className="flex items-center justify-between">
                <div className="text-[10px] font-jakarta text-[#6B7280] uppercase tracking-[0.1em] font-semibold">
                  {item.name}
                </div>
                {item.positive ? (
                  <div className="w-1.5 h-1.5 bg-[#6A994E] rounded-full animate-pulse" />
                ) : (
                  <div className="w-1.5 h-1.5 bg-[#BC4749] rounded-full animate-pulse" />
                )}
              </div>
              
              <div className={`text-2xl font-jakarta font-bold tracking-tight text-[#1F2933]`}>
                {item.value}
              </div>
              
              <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-jakarta font-bold ${
                item.positive 
              ...
              <svg width="100%" height="100%" viewBox="0 0 100 60" preserveAspectRatio="none">
                <defs>
                  <linearGradient id={`gradient-${index}`} x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stopColor={item.positive ? '#6A994E' : '#BC4749'} stopOpacity="0.3" />
                    <stop offset="100%" stopColor={item.positive ? '#6A994E' : '#BC4749'} stopOpacity="0" />
                  </linearGradient>
                </defs>
                {/* Area fill */}
                <path
                ...
                <path
                  ...
                  fill="none"
                  stroke={item.positive ? '#6A994E' : '#BC4749'}
                  strokeWidth="2.5"
                  strokeLinecap="round"
